...
export class ReportController {
  ...
  static async studentPerformance(req: Request, res: Response) {
    const data = await ReportService.studentPerformance(req.params.numero_identificacion, req.query as Record<string, unknown>);
  ...
  static async classroomSummary(req: Request, res: Response) {
    const data = await ReportService.classroomSummary(req.query as Record<string, unknown>);
    return sendSuccess(res, "Resumen de aula", data);
  }

  static async classroomSummaryExportCsv(req: Request, res: Response) {
    const result = await ReportService.classroomSummaryExportCsv(req.query as Record<string, unknown>);
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename=\"${result.fileName}\"`);
    return res.status(200).send(result.csv);
  }
  ...
  static async classroomSummaryExportPdf(req: Request, res: Response) {
    const result = await ReportService.classroomSummaryExportPdf(req.query as Record<string, unknown>);
  ...
  static async groupSummary(req: Request, res: Response) {
    const data = await ReportService.groupSummary(req.params.groupId, req.query as Record<string, unknown>);
    return sendSuccess(res, "Resumen por grupo", data);
  }

  static async schoolSummary(req: Request, res: Response) {
    const data = await ReportService.schoolSummary(req.params.schoolId, req.query as Record<string, unknown>);
